  ...
  const getdata = () => {
    axios
      ...
      .then(function (res) {
        ...
        let newp = Math.floor(
          (res.data.medicine[id].price * res.data.medicine[id].discount) / 100
        );
    ...
    <>
      ...
      <div id="main">
        <div id="left">
          ...
          <div class="content">
            <h2>
              ₹ {newp} <span className="discount2">₹{server.price}</span>{" "}
              <span class="discount">{server.discount}%off</span>
            </h2>
            <button
              id="addto"
              onClick={() => {
                dispatch(addItems(server));
              }}
            >
              Add to cart
            </button>
        ...
        <div id="right">
          ...
          <div className="highlight">
            ...
            <div>
              <h2 id="use" className="content2">
                Use
              </h2>
              <span>{server.uses}</span>
            </div>
            <div>
              <h2 id="benifits" className="content2">
                Benefits
              </h2>
              <ul>
                <li>{server.benifits}</li>
              </ul>
            </div>
          </div>
        </div>
