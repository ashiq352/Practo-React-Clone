...
export default function MedicinesProduct() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => setData(res.data.medicine));
  }, []);
  ...
  useEffect(() => {
    const disp = data.filter((e) => {
      ...
      if (skye.startsWith(`${prodname}`) || skye.endsWith(`${prodname}`))
        return e;
    });
  ...
  return (
    <div className="MedicinesProduct MedicinesProductFlex">
      <div id="MedicinesProductCategory">
        <div className="MedicinesProductCategoryTitle1">
          <Link to={"/medicines"} className="backColor">
            Home{">"}
          </Link>
          <b>{prodname}</b>
        </div>
        ...
        <div className=" MedicinesProductCategoryInfo">
          <div
            className="MedicinesProductFlex MedicineCatDrop"
            onClick={() => {
              // console.log("here")
      ...
      <div className="MedicinesProductProduct">
        <div className="MedicinesProductProdCate MedicinesProductFlex">
          <div>
            <h1 className="headName">{prodname}</h1>
          </div>
          <div>
            <p>11 products</p>
          </div>
          ...
          {filterdata.map((e) => (
            <div className="trial">
              <Link to={`/description/${e.id}`}>
                <>
                  ...
                  <div className="MedicinesProductProductProdListNameANDPrice">
                    <div className="MedicinesProductProductProdListName">
                      {e.tablet}
                    </div>
                    <div className="MedicinesProductPriceAND">
                      {/* var disPrice = ({e.price} * ({e.discount}/100)) */}
                      {/* console.log(disPrice); */}
                      <div className="MedicinesProductPrice">
                        <span>₹</span>{" "}
                        <span>
                          {(e.price - e.price * (e.discount / 100)).toFixed(2)}
                        </span>
                      </div>
                      <div className="MedicinesProductOffer">
                        {" "}
                        <span>₹</span>
                        <span>{e.price}</span>{" "}
                      </div>
                      <span className="MedicinesProductOfferPrice">
                        {e.discount}% off
                      </span>
                    </div>
                  </div>
              ...
              <div className="MedicinesProductAddTo MedicinesProductFlex">
                ...
                <div className="MedicinesProductAddToButton">
                  <button
                    onClick={() => {
                      dispatch(addItems(e));
                    }}
